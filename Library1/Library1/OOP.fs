...
type Shape = {Reposition:Point->unit; Draw:unit->unit}
...
let makeShape initPos draw = 
    ...
    {Reposition = 
     ...
     Draw = 
         //draw the shape passing the current position to give draw function
         (fun()->draw !currPos);
    }
...
let circle initPos = makeShape initPos (draw "Circle")

//creates a new square shape
let square initPos = makeShape initPos (draw "Square")

//list of shapes in their initial positions
let shapes = [ circle(new Point(10,10)); square(new Point(30,30)) ]

//draw all the shapes
let drawShapes() = shapes |> List.iter(fun s -> s.Draw())
